...
import com.crossover.aws.representaion.ReadDataPoint;
...
public class WeatherClient {
    ...
    public void addAirport(String iata, Double lat, Double lon) {
        WebTarget path = collect.path("/airport/" + iata + "/" + lat.toString() + "/" + lon.toString());
    ...
    public void populate(String iata, String type, int mean, int first, int second, int third, int count) {
        WebTarget path = collect.path("/weather/" + iata + "/" + type);
        ReadDataPoint dp = new ReadDataPoint(mean, first, second, third, count);
    ...
    public static void main(String[] args) {
        ...
        wc.pingCollect();
        wc.populate("BOS", "wind", 10, 5, 10, 15, 20);
        wc.query("BOS");
        wc.query("JFK");
        wc.query("EWR");
        wc.query("LGA");
